...
int32_t readfromspi(uint16_t headerLength, uint8_t *headerBuffer, uint16_t readlength, uint8_t *readBuffer)
{
    ...
    /* STM32H7's SPI peripheral (SPIv3) has no legacy DR register to poke byte-by-byte like
     * F4's SPIv1/v2 - it uses a TXDR/RXDR FIFO instead, and its HAL_SPI_Receive() does not
     * have the single-byte-read quirk the F4 HAL had, so just use it directly. It clocks out
     * whatever TXDR currently holds (0 after reset/last use), satisfying the same "set MOSI
     * to 0 during read" requirement noted in the F4 platform this was ported from. */
    if (readlength != 0)
    {
